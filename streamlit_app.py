...
PERSIST_KEYS = {
    ...
    "ack_0","ack_1","ack_2","ack_3","ack_4"   # נשמור גם את תיבות האישור עצמן
}
...
# =========================
# עטיפות ווידג'טים עם שמירה אוטומטית (on_change)
# =========================
...
def ti(label, key, **kw):      # text_input
    kw.setdefault("value", st.session_state.get(key, ""))
    kw["key"] = key
    kw.setdefault("on_change", _persist_on_change(key))
    return st.text_input(label, **kw)

def ta(label, key, **kw):      # text_area
    kw.setdefault("value", st.session_state.get(key, ""))
    kw["key"] = key
    kw.setdefault("on_change", _persist_on_change(key))
    return st.text_area(label, **kw)

def sb(label, options, key, **kw):  # selectbox
    current = st.session_state.get(key)
    if current in options:
        kw.setdefault("index", options.index(current))
    kw["key"] = key
    kw.setdefault("on_change", _persist_on_change(key))
    return st.selectbox(label, options, **kw)

def ms(label, options, key, **kw):  # multiselect
    kw.setdefault("default", st.session_state.get(key, []))
    kw["key"] = key
    kw.setdefault("on_change", _persist_on_change(key))
    return st.multiselect(label, options, **kw)

def ra(label, options, key, **kw):  # radio
...
def ni(label, key, **kw):      # number_input
    if "value" not in kw:
        kw["value"] = st.session_state.get(key, kw.get("min_value", 0))
    kw["key"] = key
    kw.setdefault("on_change", _persist_on_change(key))
    return st.number_input(label, **kw)

def cb(label, key, **kw):      # checkbox
    kw.setdefault("value", bool(st.session_state.get(key, False)))
    kw["key"] = key
    kw.setdefault("on_change", _persist_on_change(key))
...
if is_admin_mode:
    ...
    pwd = ti("סיסמת מנהל", key="admin_pwd_input", type="password")
...
if step == 0:
    ...
    st.markdown("---")
    cb("אני מצהיר/ה כי מילאתי פרטים אישיים באופן מדויק. *", key="ack_0")
    st.session_state.acks[0] = bool(st.session_state.get("ack_0", False))
    save_persisted_state()
...
if step == 1:
    ...
    # תחום מוביל – תמיד מתוך השלושה שנבחרו
    options_top = ["— בחר/י —"] + (st.session_state.chosen_domains or [])
    sb("מה התחום הכי מועדף עליך, מבין שלושתם? *", options_top, key="top_domain")
    ...
    for i in range(1, RANK_COUNT + 1):
        with cols[(i - 1) % 2]:
            ...
            # נשתמש ב-selectbox הרגיל אך דרך עטיפה sb כדי שיישמר on_change
            sb(f"מקום הכשרה {i} (בחר/י מוסד) *", opts, key=f"rank_{i}")

    ta("האם קיימת בקשה מיוחדת הקשורה למיקום או תחום ההתמחות? *", key="special_request", height=100)

    st.markdown("---")
    cb("אני מצהיר/ה כי העדפתי הוזנו במלואן. *", key="ack_1")
    st.session_state.acks[1] = bool(st.session_state.get("ack_1", False))
    save_persisted_state()
...
if step == 2:
    ...
    st.markdown("---")
    cb("אני מצהיר/ה כי הממוצע שהזנתי נכון. *", key="ack_2")
    st.session_state.acks[2] = bool(st.session_state.get("ack_2", False))
    save_persisted_state()
...
if step == 3:
    ...
    st.markdown("---")
    cb("אני מצהיר/ה כי מסרתי מידע מדויק על התאמות. *", key="ack_3")
    st.session_state.acks[3] = bool(st.session_state.get("ack_3", False))
    save_persisted_state()
...
if step == 4:
    ...
    st.markdown("---")
    cb("אני מצהיר/ה כי עניתי בכנות על שאלות המוטיבציה. *", key="ack_4")
    st.session_state.acks[4] = bool(st.session_state.get("ack_4", False))
    save_persisted_state()
...
if submitted:
    ...
    if errors:
        ...
        try:
            ...
            # נקה קובץ state כדי לא לעלות טיוטה בהפעלה הבאה
            if STATE_FILE.exists():
                try:
                    STATE_FILE.unlink()
                except Exception:
